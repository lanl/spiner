...
namespace Spiner {
  ...
  public:
    ...
    // 1D interpolation
    PORTABLE_INLINE_FUNCTION Real operator() (const Real x,
                                              const PortableMDArray<Real>& A) const {
      int ix;
      weights_t w;
      weights(x,ix,w);
    ...
    inline herr_t saveHDF(hid_t loc, const std::string& name) const {
      ...
      status = H5LTmake_dataset(loc, name.c_str(),
                                SP5::RG1D::RANGE_RANK,
                                range_dims,
                                H5T_REAL, range);
      status += H5LTset_attribute_int(loc, name.c_str(),
                                      SP5::RG1D::N,
                                      &n, 1);
      status += H5LTset_attribute_string(loc, name.c_str(),
                                         SP5::RG1D::RANGE_INFONAME,
                                         SP5::RG1D::RANGE_INFO);
      return status;
    }
